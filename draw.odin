...
DrawTexelFlatShaded :: proc(
	...
	if depth <= zBuffer[zIndex] {
		interpU :=
			((uv1.x * p1.z) * alpha + (uv2.x * p2.z) * beta + (uv3.x * p3.z) * gamma) * depth
		interpV :=
			((uv1.y * p1.z) * alpha + (uv2.y * p2.z) * beta + (uv3.y * p3.z) * gamma) * depth
...
DrawPixelPhongShaded :: proc(
	x, y: f32,
	v1, v2, v3, n1, n2, n3, p1, p2, p3: ^Vector3,
	color: rl.Color,
	light: Light,
	zBuffer: ^ZBuffer,
	ambient: f32 = 0.2,
...
DrawTexturedTriangleFlatShaded :: proc(
	...
	if p1.y != p2.y {
		...
		for y := p1.y; y <= p2.y; y += 1 {
			...
			for x := xStart; x <= xEnd; x += 1 {
				DrawTexelFlatShaded(x, y, p1, p2, p3, uv1, uv2, uv3, texture, intensity, zBuffer)
			}
...
DrawTexturedUnlit :: proc(
	...
	for &tri in triangles {
		...
		DrawTexturedTriangleFlatShaded(&p1, &p2, &p3, &uv1, &uv2, &uv3, texture, 1.0, zBuffer)
